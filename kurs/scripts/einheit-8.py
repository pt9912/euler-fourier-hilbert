...
# Randbereich, in dem die Hilbert-Transformation durch das endliche Signalfenster
# verzerrt wird. ~3 % der Gesamtdauer reichen für eine deutliche, ehrliche Markierung.
edge_fraction = 0.03
edge_left = T * edge_fraction
edge_right = T * (1 - edge_fraction)
...
axes[0].axvspan(0, edge_left, color="gray", alpha=0.15)
axes[0].axvspan(edge_right, T, color="gray", alpha=0.15,
                label="Randbereich (Hilbert-Artefakte)")
...
axes[1].axvspan(0, edge_left, color="gray", alpha=0.15)
axes[1].axvspan(edge_right, T, color="gray", alpha=0.15)
